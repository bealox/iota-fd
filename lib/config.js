...
let configFilePath = (process.env[(process.platform == 'win32') ? 'USERPROFILE' : 'HOME'] || process.cwd()) + '/iota-fd.conf';
...
function processAnswers(answers){
	...
	config.uid = helper.randomNumber();
...
function questions(){
	return inquirer.prompt([
		...
		},
		{
			...
			validate(input) {
				if (input.length < 5)
					return 'You must type in an username with more than 5 characters. Try again.';
				return true;
			}
		},
		{
